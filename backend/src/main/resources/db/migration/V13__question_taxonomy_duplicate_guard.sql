ALTER TABLE question ADD COLUMN root_taxonomy_node_id UUID REFERENCES taxonomy_node(id);
ALTER TABLE question ADD COLUMN child_taxonomy_node_id UUID REFERENCES taxonomy_node(id);
ALTER TABLE question ADD COLUMN normalized_question_text TEXT;
...
CREATE UNIQUE INDEX uq_question_root_child_text
    ON question(root_taxonomy_node_id, child_taxonomy_node_id, normalized_question_text)
    WHERE root_taxonomy_node_id IS NOT NULL
      AND child_taxonomy_node_id IS NOT NULL
      AND normalized_question_text IS NOT NULL;
